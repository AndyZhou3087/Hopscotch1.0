...
/**
 * 创建游戏角色时调用，实际调用AdTracking的创建角色接口
 * roleName: 游戏角色名
 */
+ (void)onCreateRole:(NSString *)roleName;

/**
 * 充值成功时调用，实际调用GoogleAdwords、InMobi和AdTracking的充值统计接口
 * orderId: 订单ID，可为空
 * amount: 充值金额（单位分，即无论什么币种，都需要乘以100）
 * currencyType: 货币类型，可为空，参考：人民币 CNY，美元 USD；欧元 EUR
 * payment: 支付方式，可为空，如：支付宝
 */
+ (void)chargeTo3rd:(NSString *)orderId amount:(NSInteger)amount currencyType:(NSString *)currencyType payment:(NSString *)payment;
