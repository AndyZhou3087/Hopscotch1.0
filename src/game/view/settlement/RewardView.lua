...
function RewardView:getReward(parameters)
    ...
    m_effect:setPosition(fireworkPos[1]) 
    local i = math.random(1,#fireworkOrder)
    self.handler1 = Tools.delayCallFunc(0.3,function()
        local m_effect=cc.ParticleSystemQuad:create("effect/cd.plist"):addTo(self)
        m_effect:setAutoRemoveOnFinish(false)
        m_effect:setPosition(fireworkPos[fireworkOrder[i][1]])
    end)
    self.handler2 = Tools.delayCallFunc(0.6,function()
        local m_effect=cc.ParticleSystemQuad:create("effect/cd.plist"):addTo(self)
        m_effect:setAutoRemoveOnFinish(false)
        m_effect:setPosition(fireworkPos[fireworkOrder[i][2]])
    end)
    self.handler3 = Tools.delayCallFunc(0.9,function()
        local m_effect=cc.ParticleSystemQuad:create("effect/cd.plist"):addTo(self)
        m_effect:setAutoRemoveOnFinish(false)
        m_effect:setPosition(fireworkPos[fireworkOrder[i][3]])
    end)
    self.handler4 = Tools.delayCallFunc(1.2,function()
        local m_effect=cc.ParticleSystemQuad:create("effect/cd.plist"):addTo(self)
        m_effect:setAutoRemoveOnFinish(false)
        m_effect:setPosition(fireworkPos[fireworkOrder[i][4]])
    end)
